"""
This is a complete version of the code_tokenize project adapted for this artifact.
The original project can be found at: https://github.com/cedricrupb/code_tokenize
"""
from .parsers   import ASTParser

from .tokenizer import tokenize_tree
from .config    import create_tokenization_config
...
def tokenize(source_code, lang = "guess", **kwargs):
    ...
    if lang != "java":  raise ValueError("Only java is supported!")

    logger.debug("Parses source code with parser for %s" % lang)

    # Setup config
    config = create_tokenization_config()
